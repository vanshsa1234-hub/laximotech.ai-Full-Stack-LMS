...
export function useCommunityFeed() {
  ...
  return useQuery({
    ...
    queryFn:  () => communityApi.feed().then(r => r.data),
    enabled:  !!session,
...
export function useCreatePost() {
  const queryClient = useQueryClient();
  return useMutation({
    mutationFn: (data: { content?: string; mediaUrl?: string; mediaType?: string }) => communityApi.createPost(data),
...
export function useCommunityMembers(search?: string) {
  ...
  return useQuery({
    ...
    queryFn:  () => communityApi.members(search ? { search } : {}).then(r => r.data),
    enabled:  !!session,
  });
}
...
export function useCommunityProfile(userId: string | null) {
  ...
  return useQuery({
    ...
    queryFn:  () => communityApi.profile(userId as string).then(r => r.data),
...
export function useSendFriendRequest() {
  const queryClient = useQueryClient();
  return useMutation({
    mutationFn: (receiverId: string) => communityApi.sendFriendRequest(receiverId),
    onSuccess: (_r, receiverId) => {
      queryClient.invalidateQueries({ queryKey: ['community-friend-requests'] });
      queryClient.invalidateQueries({ queryKey: ['community-profile', receiverId] });
    },
  });
}
...
export function useFriendRequests() {
  const { data: session } = useSession();
  return useQuery({
    queryKey: ['community-friend-requests'],
    queryFn:  () => communityApi.friendRequests().then(r => r.data),
    enabled:  !!session,
...
export function useRespondFriendRequest() {
  ...
  return useMutation({
    mutationFn: ({ id, accept }: { id: string; accept: boolean }) =>
      accept ? communityApi.acceptFriendRequest(id) : communityApi.rejectFriendRequest(id),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['community-friend-requests'] });
...
export function useCommunityFriends() {
  ...
  return useQuery({
    ...
    queryFn:  () => communityApi.friends().then(r => r.data),
    enabled:  !!session,
  });
}
...
export function useConversations() {
  ...
  return useQuery({
    ...
    queryFn:  () => communityApi.conversations().then(r => r.data),
    enabled:  !!session,
    refetchInterval: 8000,
  });
}
...
export function useThread(otherUserId: string | null) {
  const { data: session } = useSession();
  return useQuery({
    queryKey: ['community-thread', otherUserId],
    queryFn:  () => communityApi.thread(otherUserId as string).then(r => r.data),
...
export function useSendMessage() {
  const queryClient = useQueryClient();
  return useMutation({
    mutationFn: (data: { receiverId: string; content?: string; mediaUrl?: string; mediaType?: string }) =>
      communityApi.sendMessage(data),
    onSuccess: (_r, vars) => {
      queryClient.invalidateQueries({ queryKey: ['community-thread', vars.receiverId] });
