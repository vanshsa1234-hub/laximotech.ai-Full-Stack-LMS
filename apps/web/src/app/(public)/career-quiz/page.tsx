...
const DEFAULT_QUESTIONS = [
  ...
  {
    id: 'salary',
    question: 'What is your expected salary range (per year)?',
    emoji: '💰',
    options: [
...
function getRecommendation(answers: Record<string, string>): Recommendation[] {
  ...
  if (interest === 'tech' || interest === 'data') {
    recs.push({
      ...
      why: 'AI/ML is India\'s fastest-growing career. Your tech interest plus your ' + (salary === 'top' || salary === 'senior' ? 'high salary goal' : 'goals') + ' make this a perfect match.',
  ...
  if (interest === 'data' || goal === 'job') {
    recs.push({
      ...
      why: 'Data Analytics has good jobs even at entry level. You can be job-ready in just 4 months.',
  ...
  if (interest === 'security') {
    recs.push({
      ...
      match: 96,
      why: 'Demand in cybersecurity is huge and supply is low. With your interest in security, this is a perfect fit.',
  ...
  if (interest === 'build') {
    recs.push({
      ...
      match: 91,
      why: 'Enjoy building things? In Full Stack development, you build websites and apps that lakhs of people use.',
      firstCourse: { title: 'Python Programming', slug: 'python-programming-hindi' },
    });
  ...
  if (recs.length === 0) {
    recs.push({
      ...
      why: 'AI Engineering is India\'s most in-demand career. It\'s a safe bet from any background.',
      firstCourse: { title: 'Python Programming', slug: 'python-programming-hindi' },
    });
  }
        ...
        <div className="bg-mesh pt-28 pb-14 text-center">
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}>
            ...
            <h1 className="font-heading font-bold text-white text-4xl md:text-5xl mb-3">
              {heroTitle ?? <>Which Career Is <span className="text-brand-orange">Best For You?</span></>}
            </h1>
            <p className="text-white/70 text-lg">{heroSubtitle ?? 'Find out in 5 questions — AI will suggest the best path for you'}</p>
          </motion.div>
        </div>
