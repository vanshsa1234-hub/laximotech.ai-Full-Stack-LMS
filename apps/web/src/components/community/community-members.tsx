    ...
    <div>
      <div className="relative mb-4">
        ...
        <input
          ...
          className="w-full pl-9 pr-3 py-2.5 rounded-xl border border-gray-100 dark:border-gray-700/60 bg-white dark:bg-gray-900/60 text-sm text-gray-800 dark:text-gray-100 placeholder:text-gray-400 dark:placeholder:text-gray-500 focus:outline-none focus:ring-2 focus:ring-brand-blue/30"
        />
      </div>
      ...
      {isLoading ? (
        ...
        <div className="text-center py-16 bg-white dark:bg-gray-900/60 rounded-2xl border border-gray-100 dark:border-gray-700/60 shadow-card">
          <p className="text-gray-400 dark:text-gray-500 text-sm">No members found.</p>
        </div>
      ) : (
        <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
          {members.map((m: any) => (
            <button
              ...
              className="bg-white dark:bg-gray-900/60 rounded-2xl border border-gray-100 dark:border-gray-700/60 shadow-card p-4 flex flex-col items-center text-center hover:border-brand-blue/30 hover:shadow-blue transition-all"
            >
              ...
              <div className="text-sm font-semibold text-gray-900 dark:text-gray-100 truncate w-full flex items-center justify-center gap-1">
                {m.name ?? 'Learner'}
                {m.role !== 'STUDENT' && <ShieldCheck size={12} className="text-brand-orange flex-shrink-0" />}
              </div>
              <div className="text-[11px] text-gray-400 dark:text-gray-500 flex items-center gap-1 mt-1">
