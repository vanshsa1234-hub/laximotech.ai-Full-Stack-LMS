      ...
      {userId && (
        ...
        >
          <motion.div
            ...
            className="bg-white dark:bg-gray-900/90 rounded-2xl shadow-card border border-gray-100 dark:border-gray-700/60 w-full max-w-sm overflow-hidden"
          >
            ...
            {isLoading || !profile ? (
              ...
              <div className="px-6 pb-6">
                <div className="w-16 h-16 rounded-full bg-brand-blue -mt-8 border-4 border-white dark:border-gray-900 flex items-center justify-center text-white font-bold text-xl overflow-hidden">
                  {profile.image ? <img src={profile.image} alt="" className="w-full h-full object-cover" /> : (profile.name?.[0] ?? '?')}
                </div>

                <h3 className="font-heading font-bold text-gray-900 dark:text-gray-100 text-lg mt-2">{profile.name}</h3>
                ...
                {profile.bio && <p className="text-sm text-gray-500 dark:text-gray-400 mt-2">{profile.bio}</p>}

                <div className="grid grid-cols-4 gap-2 mt-4">
                  <div className="bg-gray-50 dark:bg-gray-800/70 rounded-xl p-2 text-center">
                    <BookOpen size={16} className="mx-auto text-brand-blue mb-1" />
                    <div className="font-bold text-gray-900 dark:text-gray-100 text-sm">{profile.enrolledCourses}</div>
                    <div className="text-[10px] text-gray-400 dark:text-gray-500">Courses</div>
                  </div>
                  <div className="bg-gray-50 dark:bg-gray-800/70 rounded-xl p-2 text-center">
                    <Award size={16} className="mx-auto text-brand-orange mb-1" />
                    <div className="font-bold text-gray-900 dark:text-gray-100 text-sm">{profile.certificates}</div>
                    <div className="text-[10px] text-gray-400 dark:text-gray-500">Certs</div>
                  </div>
                  <div className="bg-gray-50 dark:bg-gray-800/70 rounded-xl p-2 text-center">
                    <Trophy size={16} className="mx-auto text-yellow-500 mb-1" />
                    <div className="font-bold text-gray-900 dark:text-gray-100 text-sm">#{profile.rank}</div>
                    <div className="text-[10px] text-gray-400 dark:text-gray-500">Rank</div>
                  </div>
                  <div className="bg-gray-50 dark:bg-gray-800/70 rounded-xl p-2 text-center">
                    <Zap size={16} className="mx-auto text-purple-600 mb-1" />
                    <div className="font-bold text-gray-900 dark:text-gray-100 text-sm">{profile.avgProgress}%</div>
                    <div className="text-[10px] text-gray-400 dark:text-gray-500">Progress</div>
                  </div>
                </div>
                ...
                <div className="mt-5 flex gap-2">
                  {profile.friendStatus === 'SELF' ? null : (profile.friendStatus === 'FRIENDS' || canMessageDirectly) ? (
                    ...
                    <button disabled className="flex-1 flex items-center justify-center gap-2 bg-gray-100 dark:bg-gray-800/70 text-gray-400 dark:text-gray-500 text-sm font-semibold py-2.5 rounded-xl cursor-not-allowed">
                      <Clock size={16} /> Request sent
                    </button>
                  ) : profile.friendStatus === 'REQUEST_RECEIVED' ? (
                    <div className="flex-1 text-center text-sm text-gray-400 dark:text-gray-500 py-2.5">Check your Friend Requests tab</div>
                  ) : (
                    <button
